...
impl GeneralAfterParameter<f64> for MinParameter {
    fn after(
        &self,
        ctx: GeneralParameterContext<'_>,
        _internal_state: &mut Option<Box<dyn ParameterState>>,
...
/// Builder for creating a [`MinParameter`].
#[derive(Debug)]
...
impl MinParameterBuilder {
    /// Create a new builder for [`MinParameter`] that is evaluated in "before" phase.
    ...
    /// Create a new builder for [`MinParameter`] that is evaluated in "after" phase.
    ...
    /// Create a new builder for [`MinParameter`] that is evaluated in both "before" and "after" phases.
