    ...
    fn split_value_into_periods(&self, value: PeriodValue<f64>) -> Vec<PeriodValue<f64>> {
        ...
        while current_date < end_date {
            ...
            let current_duration = if start_of_next_period <= end_date {
                start_of_next_period.duration_since(current_date)
            } else {
                end_date.duration_since(current_date)
            };
        ...
        if let Some(current_values) = self.current_values.as_mut() {
            ...
            if agg_freq.is_date_in_period(&current_period_start, &value.start) {
                ...
                let agg_period = if let Some(agg_value) = agg_func.calc_period_values(current_values) {
                    let agg_duration = value.start.duration_since(current_period_start);
        ...
        if let Some(current_values) = &self.current_values {
            if let Some(agg_value) = agg_func.calc_period_values(current_values) {
                ...
                let current_period_end = current_values
                    ...
                    .expect("Aggregation state contains no values when at least one is expected.")
                    .start;
                let current_period_duration = current_period_end.duration_since(current_period_start);
...
mod tests {
    ...
    fn test_periodic_aggregator() {
        ...
        let agg_value = agg.process_value(&mut state, PeriodValue::new(start, PywrDuration::from_days(1), 1.0));
    ...
    fn test_nested_aggregator() {
        ...
        for _i in 0..365 * 3 {
            let value = PeriodValue::new(date, PywrDuration::from_days(1), date.year() as f64);
    ...
    fn test_sub_daily_aggregation() {
        let values = vec![
            PeriodValue::new(date(2023, 1, 1).at(0, 0, 0, 0), PywrDuration::from_hours(1), 2.0),
            PeriodValue::new(date(2023, 1, 1).at(1, 0, 0, 0), PywrDuration::from_hours(2), 1.0),
            PeriodValue::new(date(2023, 1, 1).at(3, 0, 0, 0), PywrDuration::from_hours(1), 3.0),
        ];
