...
use std::simd::{f64x4, LaneCount, Simd, SimdElement, SimdFloat, StdFloat, SupportedLaneCount};
...
pub fn primal_feasibility(
    ...
    normr.sqrt() / (f64x4::splat(1.0) + normx.sqrt())
}
...
pub fn dual_feasibility(
    at: &Matrix<f64, 4>, // Sparse A matrix
    y: &[f64x4],
    c: &[f64x4],
    z: &[f64x4],
) -> f64x4 {
    ...
    norms.sqrt() / (f64x4::splat(1.0) + normy.sqrt())
}
