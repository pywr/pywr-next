...
model_tests! {
    ...
    test_mutual_exclusivity1: ("mutual-exclusivity1.json", vec!["mutual-exclusivity1.csv"], vec!["clp", "ipm-simd"]),
    test_mutual_exclusivity2: ("mutual-exclusivity2.json", vec!["mutual-exclusivity2.csv"], vec!["clp", "ipm-simd"]),
    test_mutual_exclusivity3: ("mutual-exclusivity3.json", vec!["mutual-exclusivity3.csv"], vec!["clp", "ipm-simd"]),
    test_link_with_soft_min: ("link_with_soft_min.json", vec![], vec!["ipm-simd"]),
    test_link_with_soft_max: ("link_with_soft_max.json", vec![], vec!["ipm-simd"]),
    test_delay1: ("delay1.json", vec!["delay1-expected.csv"], vec![]),
    test_loss_link1: ("loss_link1.json", vec!["loss_link1-expected.csv"], vec!["ipm-simd"]),
    test_loss_link2: ("loss_link2.json", vec!["loss_link2-expected.csv"], vec!["ipm-simd"]),
    ...
    test_river_loss1: ("river_loss1.json", vec!["river_loss1-expected.csv"], vec!["ipm-simd"]),
    test_river_gauge1: ("river_gauge1.json", vec![], vec![]),
    test_river_split_with_gauge1: ("river_split_with_gauge1.json", vec![], vec![]),
    test_thirty_day_licence: ("30-day-licence.json", vec![], vec!["ipm-simd"]),
    test_wtw1: ("wtw1.json", vec!["wtw1-expected.csv"], vec!["ipm-simd"]),
    test_wtw2: ("wtw2.json", vec!["wtw2-expected.csv"], vec!["ipm-simd"]),
